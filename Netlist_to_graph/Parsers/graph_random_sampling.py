...
for circuit, bit_approx in circuit_approxlevel.items():
    ...
    for c in circs:
        ...
        circuit_nodes = list(gr.nodes)

        primary_outputs = find_primary_outputnodes(gr)
        for po in primary_outputs[len(primary_outputs)-bit_approx[1]:]:
            number_of_nodes_to_drop = len(drop_predecessors(gr, po, []))
            nodes_to_drop.append(sorted(random.choices(circuit_nodes, k=number_of_nodes_to_drop)))
